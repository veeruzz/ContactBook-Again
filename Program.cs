    ...
    private List<List<Contact>> FindDuplicatedContacts()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = i + 1; j < n; j++)
            {
                ...
                bool sameName = !string.IsNullOrWhiteSpace(c1.firstName) &&
                ...
                bool samePhone = !string.IsNullOrWhiteSpace(c1.phoneNumber) &&
                                 c1.phoneNumber == c2.phoneNumber;

                bool sameEmail = !string.IsNullOrWhiteSpace(c1.email) &&
        ...
        var result = groups.Values.Where(g => g.Count > 1).ToList();
